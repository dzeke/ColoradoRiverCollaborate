...
Func_ACF_Lag123 <- function (ScenarioList){
  ...
  title(main="Autocorrelation Function (ACF) at lags 1 to 3", adj=0.5, line=1.75)
  ...
  for (s in 1:n_Scenario){
    ...
    HeatmapValues$Min[s] <- min(ACF_Scenario$lag1)
    ...
    HeatmapValues$Max[s] <- max(ACF_Scenario$lag1)
