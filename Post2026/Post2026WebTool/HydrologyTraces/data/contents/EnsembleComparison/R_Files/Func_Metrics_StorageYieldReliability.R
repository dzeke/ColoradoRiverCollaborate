...
Func_StorageYieldReliability <- function(ScenarioList, 
                                         YieldFraction=seq(7, 15, by=1)/15,   ## YieldFraction=seq(0.5, 1, by=0.05)
                                         ReferenceMean=15, 
  ...
  HeatmapValues_SY <- data.frame(matrix(nrow=nScenarios, ncol=9))
  colnames(HeatmapValues_SY) <- c("Scenario", "Min", "Q10", "Q25", "Median", "Q75", "Q90", "Max", "Hist")
  HeatmapValues_SY$Scenario <- ScenarioList[,2]
  
  HeatmapValues_SR <- data.frame(matrix(nrow=nScenarios, ncol=9))
  colnames(HeatmapValues_SR) <- c("Scenario", "Min", "Q10", "Q25", "Median", "Q75", "Q90", "Max", "Hist")
  ...
  for (s in 1:nScenarios){
    ...
    HeatmapValues_SY$Min[s] <- min(as.numeric(StorageCapacityMax[ii,-1:-2]))
    ...
    HeatmapValues_SY$Max[s] <- max(as.numeric(StorageCapacityMax[ii,-1:-2]))
    ...
    par(mfrow=c(1,2), mar=c(5,5,4,3), mgp=c(3, 1, 0)) ## 2x2=4 plots
    ...
    if(ntraces!=1){  ### when the scenario has more than one trace
      ...
      ## minor axis tick
      axis(1, at=seq(0,300, 10), labels=NA, tck=0.01) 
      # axis(4, at=c(1:(length(YieldFraction))), labels= NA )
      # axis(4, at=c(1:(length(YieldFraction))), labels= YieldFraction*100 , cex.axis=1.2, las=1)
      # axis(4, at=c(1:(length(YieldFraction))), labels= round(bx[1,]*ReferenceMean, digits=2) , cex.axis=1.2, las=1)
    ...
    #### plot --------------------------
    # dev.new()
    par(mar=c(5,5,4,3), mgp=c(3, 1, 0))  ## marging for second plot
    plot(x=cdf$cdf, y=cdf$sort,
         ...
         cex.main=1.1,
         xlab="Reliability of meeting yield",
         ylab="Storage (maf)",
         ylim=c(0,300),
         cex.lab=1.3,
         cex.axis=1.2,
    ...
    axis(2, at=seq(0,300, 10), labels=NA, tck=0.01)
    axis(4, labels=NA)
    axis(4, at=seq(0,300, 10), labels=NA, tck=0.01)
    ...
    legend("topleft", legend=paste0("Yield = ", round(ReferenceMean*DesiredYieldFraction, digits=2), " maf "),
           cex=1.2, bty="n", inset=c(0.05, 0.05))
  ...
  assign(paste0("HM_SYR",DesiredYieldFraction*ReferenceMean), HeatmapValues_SY, .GlobalEnv)
