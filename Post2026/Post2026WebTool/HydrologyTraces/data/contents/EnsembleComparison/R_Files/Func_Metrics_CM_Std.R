...
Func_Std <- function(ScenarioList, duration=10){
  ...
  if (nx_max>2){  ## it means that want metric over a duration along with the full period
    ...
    title(main="Standard Deviation (StD)", adj=0.5, line=1.75)
  ...
  ## add line
  abline(h=HistMetric, col="red", lwd=2, lty=5)  ## 1906-2021: 4.31 maf/yr
  
  legend("topright",
         legend=paste0("Historical standard deviation    \n (", yr1, "-", yr2, "): ", HistMetric, " maf/yr  " ),
         cex=0.9,
         col=c("red" ),
         lwd=1.5, lty=c(5), 
         inset=c(0.02, 0.03), 
