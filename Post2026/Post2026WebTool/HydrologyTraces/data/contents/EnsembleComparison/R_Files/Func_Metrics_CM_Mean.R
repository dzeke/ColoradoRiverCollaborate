...
Func_Mean <- function(ScenarioList, duration=10){
  ...
  HeatmapValues <- data.frame(matrix(nrow=nScenarios, ncol=10))
  colnames(HeatmapValues) <- c("Scenario", "Min", "Q10", "Q25", "Median", "Q75", "Q90", "Max", "Hist", "Mean")
  ...
  if (nx_max>2){  ## it means that want metric over a duration along with the full period
    ...
    HeatmapValues$Mean[1] <- mean(Dist_ComMetric$full)
    ...
    title(main="Mean", adj=0.5, line=1.75)
    ...
    if(nScenarios>1){
      ...
      for (s in 2:nScenarios){
        ...
        HeatmapValues$Mean[s] <- mean(Dist_ComMetric$full)
  ...
  ltm <- round(mean(Hist[,2]), digits=2)
  MillMean <- round( mean(Hist[which( Hist[,1] >=2000 & Hist[,1]<=yr2), 2]), digits=2)
  ...
  ## add legend
  legend("topright",
         legend=c(paste0("Historical long-term mean  \n (", yr1, "-", yr2, "): ", ltm," maf/yr  ") , 
                  paste0("Millennium drought mean \n  (2000-", yr2, "): ", MillMean," maf/yr  ") ),
         cex=0.9,
         col=c("red", "red" ),
         lwd=1.5, lty=c(1, 5), 
         inset=c(0.02, 0.03), 
