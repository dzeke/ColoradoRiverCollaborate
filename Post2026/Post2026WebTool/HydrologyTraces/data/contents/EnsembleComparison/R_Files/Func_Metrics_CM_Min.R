...
Func_Min <- function(ScenarioList, duration=10){
  ...
  if (nx_max>2){  ## it means that want metric over a duration along with the full period
    ...
    title(main="Minimum (1-yr minimum)", adj=0.5, line=1.75)
  ...
  HistMetric <- round(min(Hist[,2]), digits=2)
  HistMeyricYr <- Hist[which (Hist[,2] == min(Hist[,2])) , 1]
  
  ## add line
  abline(h=HistMetric, col="red", lwd=2, lty=5)  ## 1977: 5.5 maf/yr
   
  
  ## add legend
  legend("topright",
         legend=paste0("Historical minimum annual flow    \n (", HistMeyricYr, "): ", HistMetric, " maf/yr  " ),
         cex=0.9,
         col=c("red" ),
         lwd=1.5, lty=c(5), 
         inset=c(0.02, 0.03), 
