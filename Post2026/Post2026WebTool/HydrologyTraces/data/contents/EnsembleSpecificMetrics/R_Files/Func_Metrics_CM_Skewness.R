...
Func_Skewness <- function(ScenarioList, duration=10){
  ...
  ## historical metric
  HistMetric <- round(Rfast::colskewness(as.matrix(Hist$NF_WY)), digits=2)
  ...
  if (nx_max>2){  ## it means that want metric over a duration along with the full period
    ...
    y1 <- abs(floor(min(Dist_ComMetric[,-1], HistMetric)) )
    y2 <- abs(ceiling(max(Dist_ComMetric[,-1], HistMetric)) ) 
    y <- max(y1, y2)
    y1 <- -1*y
    y2 <- y
    dymax <- 1
    ...
    title(main="Skewness", adj=0.5, line=2)
