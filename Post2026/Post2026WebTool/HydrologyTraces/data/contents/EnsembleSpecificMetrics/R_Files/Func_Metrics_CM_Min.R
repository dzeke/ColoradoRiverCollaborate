...
###########################################################################################################################################################
####### Function to plot Min ranges of scenarios (decadal or any desired duration)



Func_Min <- function(ScenarioList, duration=10){
  ...
  Ranges_ComMetric <- function(scenario){  
    ...
    for(b in 1:nblock){
      ...
      if(b<nblock){
        ...
        Dist_ComMetric[,(b+1)] <- colMins(as.matrix(scenario_sub[,-1]))
    ...
    Dist_ComMetric$full <- colMins(as.matrix(scenario[,-1]))
  ...
  # Metric_AllScenariosTraces_List <- vector("list", length = nScenarios)
  ...
  HistMetric <- round(min(Hist$NF_WY), digits=2)
  ...
  if (nx_max>2){  ## it means that want metric over a duration along with the full period
    ...
    # Metric_AllScenariosTraces_List[[1]] <- Dist_ComMetric$full
    ...
    boxplot(Dist_ComMetric[,-1], xlim=c(0.5,nx+0.5),
            # main="Minimum (1-yr minimum during periods)",
            ylab="Minimum (maf/yr)",
            ylim=c(y1,y2),
            cex.lab=1.3,
            yaxt="n",
    ...
    title(main="Minimum (1-yr min)", adj=0.5, line=2)
    ...
    if(nScenarios>1){
      ...
      for (s in 2:nScenarios){
        ...
        # Metric_AllScenariosTraces_List[[s]] <- Dist_ComMetric$full
  ...
  if (nx_max<3){
    ...
    title(main="Minimum", adj=0.5, line=2)
    text(paste0(ScenarioList[2]),
         x=(par("usr")[1] + par("usr")[2])/2,
         y=(par("usr")[4]),
         xpd=T,
    ...
    ymin <- par("usr")[3]
    # text(y=(ymin-(y1-ymin)), x=1.5, ScenarioList[1,2], srt=0, pos=2, xpd=T)
  ...
  ##########################################################################################################
  #### Add lines for the historical 1-yr minimum =======================
  ...
  ## historical min 1-yr flow
  HistMetric <- round(min(Hist$NF_WY), digits=2)
  HistMetricYr <- Hist$Yr[which (Hist$NF_WY == min(Hist$NF_WY))]
  
  ## add line
  abline(h=HistMetric, col="red", lwd=2, lty="solid")  ## 1977: 5.5 maf/yr
  ...
  legend(x=xmax, y=(ymax+ymin)/2, xjust=0, yjust = 1.2, xpd=T,
  ...
  # Metric_AllScenariosTraces_Vector <- Reduce(c, Metric_AllScenariosTraces_List)
  # sd(Metric_AllScenariosTraces_Vector)
  # mean(Metric_AllScenariosTraces_Vector)
  
  ##### add the heatmap dataframe output to Global Environment ##################################################################################
  
  assign("HM_Min", HeatmapValues, .GlobalEnv)
  
} ## end of the main function: Func_Min   ################################################################################################################
