...
Func_PACF_Lag123 <- function (ScenarioList){
  ...
  title(main="PACF at lags 1 to 3", adj=0.5, line=2)
  ...
  legend(x=xmax, y=ymax, legend= c( "Lag 1", "Lag 2", "Lag 3") , title="Ensemble",
         cex=0.8,
         pch=22, pt.bg=col_box, pt.cex=1.5, horiz = F, bty="n",
         xjust=-0.2, yjust = 1, xpd=T)
  
  legend(x=xmax, y=(ymax/2), 
         legend= c( paste0("Lag 1 = ", round(PACF1_hist, digits=2)), paste0("Lag 2 = ", round(PACF2_hist, digits=2)), paste0("Lag 3 = ", round(PACF3_hist, digits=2))) , 
