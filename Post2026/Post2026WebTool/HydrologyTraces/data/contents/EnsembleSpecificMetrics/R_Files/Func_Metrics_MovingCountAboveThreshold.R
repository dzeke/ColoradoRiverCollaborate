...
Func_MovingCountAboveThreshold <- function(data , Threshold=20 , Duration=10, ScenarioName="ISM_Full"){
  ...
  if(nTraces==1){
    ...
    ymin <- par("usr")[3]
    ymax <- par("usr")[4]
    text(x=xmax, y=ymax ,  labels="Count above threshold (% of Duration)", pos=4, offset=3.5, col="black", cex=1.2, srt=-90, adj=0.5, xpd=T)
    
    
    # for (j in 2:nTraces){
    #   points(x=CountAboveThreshold$EndYr , y=CountAboveThreshold[,(2+j)],  pch=19, cex=0.7)
    #   lines(x=CountAboveThreshold$EndYr , y=CountAboveThreshold[,(2+j)])
    # }
