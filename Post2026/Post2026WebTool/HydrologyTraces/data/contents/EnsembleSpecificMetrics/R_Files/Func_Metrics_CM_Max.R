...
Func_Max <- function(ScenarioList, duration=10){
  ...
  ## historical metric
  HistMetric <- round(max(Hist$NF_WY), digits=2)
  ...
  if (nx_max>2){  ## it means that want metric over a duration along with the full period
    ...
    title(main="Maximum (1-yr max)", adj=0.5, line=2)
  ...
  if (nx_max<3){
    ...
    title(main="Maximum", adj=0.5, line=2)
    text(paste0(ScenarioList[2]),
         x=(par("usr")[1] + par("usr")[2])/2,
         y=(par("usr")[4]),
         xpd=T,
  ...
  HistMetric <- round(max(Hist$NF_WY), digits=2)
  HistMetricYr <- Hist$Yr[which (Hist$NF_WY == max(Hist$NF_WY))]
  
  ## add line
  abline(h=HistMetric, col="red", lwd=2, lty="solid")  ## 1984: 24.2 maf/yr
