...
###########################################################################################################################################################
####### Function to plot Mean ranges of scenarios (decadal or any desired duration)



Func_Mean <- function(ScenarioList, duration=10){
  ...
  Ranges_ComMetric <- function(scenario){  
    ...
    for(b in 1:nblock){
      ...
      if(b<nblock){
        ...
        Dist_ComMetric[,(b+1)] <- colMeans(as.matrix(scenario_sub[,-1]))
    ...
    Dist_ComMetric$full <- colMeans(as.matrix(scenario[,-1]))
  ...
  # par(mar=c(2, 4, 3, 10) + 0.1 , mgp=c(2.7, 1, 0) )
  ...
  if (nx_max>2){  ## it means that want metric over a duration along with the full period
    ...
    title(main="Mean", adj=0.5, line=2)
    text(paste0(ScenarioList[2]),  ## paste0("Ensemble:  ", ScenarioList[2]),
    ...
    if (nx_max<3){
      ...
      if(nScenarios>1){
        ...
        for (s in 2:nScenarios){
          ...
          # text(y=(ymin-(y1-ymin)), x=s+0.5, ScenarioList[s,2], srt=0, pos=2, xpd=T)
    ...
    ##########################################################################################################
    #### Add lines for the long-term average and millennium drought average flow ===.====================
  ...
  yr2 <- max(Hist[,1])
    ...
    ltm <- round(mean(Hist$NF_WY), digits=2)
    MillMean <- round( mean(Hist$NF_WY[which( Hist$Yr>=2000 & Hist$Yr<=yr2)]), digits=2)
    
    ## store the historical value of the metric in the heatmap dataframe
    HeatmapValues$Hist <- ltm
    
    ## add lines
    abline(h=ltm, col="red", lwd=2)  ## 1906-2021: 14.65 maf/yr
    abline(h=MillMean, col="red", lwd=2, lty="longdash")  ## 2000-2021: 12.28 maf/yr   2000-2019: 12.7 maf/yr
    ...
    legend(x=xmax, y=(ymax+ymin)/2, xjust=0, yjust = 1.2, xpd=T,
           legend=c(paste0("Historical \n(", yr1, "-", yr2, "): \n", ltm," maf/yr  ") , 
                    paste0("(2000-", yr2, "): \n", MillMean," maf/yr  ") ),
           cex=0.8,
           col=c("red", "red" ),
           lwd=1.5, lty=c("solid", "dashed"), 
           # seg.len=1.5,
           # inset=c(0.02, 0.03), 
           bty = "n",
           bg = "white",
    ...
    assign("HM_Mean", HeatmapValues, .GlobalEnv)
  
  
} ## end of the main function: Func_Mean   ################################################################################################################
