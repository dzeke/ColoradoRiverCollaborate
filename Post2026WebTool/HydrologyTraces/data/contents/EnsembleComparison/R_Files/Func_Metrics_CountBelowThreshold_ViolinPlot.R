...
Func_CountBelowThreshold_ViolinPlot <- function(scenario=ISM_Full , Threshold=14.78 , Duration=c(5,10,15,20) , ScenarioName="ISM_Full"){
    ...
    # CountBelowThreshold_Durations <- data.frame(NA)
    # colnames(CountBelowThreshold_Durations) <- c("trace", "CountBT", "dur")
    ...
    for (dur in 1:length(Duration)){
      ...
      # CountBelowThreshold_matrix <- as.matrix(CountBelowThreshold)
      # CountBelowThreshold_1col <- data.frame(matrix(NA, ncol=3, nrow=n*nTraces))
      # colnames(CountBelowThreshold_1col) <- c("trace", "CountBT")
      # CountBelowThreshold_1col[(1:(nrow(CountBelowThreshold)*nTraces)),3] <- matrix(CountBelowThreshold_matrix[,-1:-2], ncol=1)
      # 
  ...
  ###### Plot ====================================================
    ...
    ggplot(CountBelowThreshold_Durations, aes(x=dur, y=CountBT))+
      ...
      ### Legend ----
      annotate("text", x=5.85, y=16, label="Boxplot: ranges of CBT", size=4)+
      annotate("rect", xmin=5.1, xmax=5.15, ymin=15.7, ymax=16.2, col=col_bxB, fill=col_bxF, lwd=0.3)+
      annotate("segment", x=5.08, xend=5.18, y=14.6, yend=14.6, col=col_density, lwd=1.2)+
      annotate("text", x=5.63, y=14.7, label="Density of CBT", size=4)+
      ...
      theme(
        ...
        plot.margin = margin(t=20, r=160, b=20, l=160)
      )
