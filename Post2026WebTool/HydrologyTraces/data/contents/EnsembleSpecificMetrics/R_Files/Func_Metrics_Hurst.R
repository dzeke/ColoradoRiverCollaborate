...
Func_Hurst <- function(ScenarioList, RoverSPlot=F, HurstBoxplot=F) {
  ...
  HurstFunc=function(Qt, RoverSPlot, add=T, plottitle="Hurst plot")
  {
    ...
    if(RoverSPlot==T){  ## do you want also the plot or just the Hurst value?
      ...
      if(!add) {
        plot(ni,RoverS,log="xy", xlab="Duration (yrs)", ylab="Rescaled Range", las=1, cex.lab=1.1, cex.axis=1.1, ylim=c(1, 20))
        axis(2, at=seq(1,100,1), labels=NA, tck=0.01)
        axis(1, at=seq(1,10000,1), labels=NA, tck=0.01)
        # title(plottitle)
        # legend(x=par("usr")[1], y=par("usr")[4], legend="Points for all traces and durations, line for each trace, \n slope of line gives Hurst exponent (H)",
        #        xjust=0, yjust=0.5, cex=1, bty="n")
        legend("topleft", legend="Points for all traces and durations, line for each \ntrace, slope of line gives Hurst exponent (H)",
               inset=c(-0.06, -0.22), cex=1, bty="n", xpd=T)
    ...
    if(RoverSPlot==T){
      lines(ni,exp(logreg[1])*ni^H,  col="gray50")
    }
  ...
  for (s in 1:nScenarios){
    ...
    for(t in 1:ntrace){
      hscen[t]=HurstFunc(scenario[,t+1], RoverSPlot, add=(t != 1), 
                         plottitle=paste0("Scenario: ", ScenarioList[s,2]) )
    }
  ...
  if (HurstBoxplot==T){
    ...
    boxplot(hlist, names=NA,  
            ylim=c(ylim1, ylim2),
            cex.axis=1.2,
            las=1,
            ylab="Hurst Coefficient",
            cex.lab=1.3,
            boxwex=0.5)
    
    # abline(h=seq(ylim1, ylim2, dy), col="grey50", lty="dotted")
    # boxplot(add=T, hlist, names=NA, las=1, cex.axis=1.2, ylim=c(ylim1, ylim2), boxwex=0.5)
    
    axis(2, at=seq(0,2, 0.1) , labels=NA, tck=0.015)
    axis(4, labels=NA)
    axis(4, at=seq(0,2, 0.1) , labels=NA, tck=0.015)
    title(main="Hurst coefficient", adj=0.5, line=2)
