...
## Figure 5B: Lake Powell Minimum Drawdown and Catastrophic Elevations - Monthly Data
...
## Figure 5C: Lake Powell Minimum Drawdown and Catastrophic Elevations - Daily Data
...
#Turn narrow into wide so separate columns for Lake Powell and Lake Mead
dfResStorageDailyWide <- pivot_wider(  dfResStorageDaily %>% group_by(ResName, Date, Year, Month) %>% select(ResName, Date, Year, Month, MonthlyValue),   names_from = ResName,   values_from = MonthlyValue)
#Filter to data after 1995
#dfResStorageWide1995 <- dfResStorageWide %>% filter(Year >= 1995)

# Filter out the last (most recent entry)
dfResStorageDailyWide$DateAsDate <- as.Date(dfResStorageDailyWide$Date)
...
dfLastDateDaily <- dfLastDateDaily[nrow(dfLastDate),] 
...
ggplot() +
  ...
  geom_point(data = dfLastDateDaily, aes(x = DateAsDate, y = `Lake Powell`), color = "blue", size = 4 ) +
